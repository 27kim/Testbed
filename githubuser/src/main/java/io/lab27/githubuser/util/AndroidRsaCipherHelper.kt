...
object AndroidCipherHelper {
    ...
    internal fun init(applicationContext: Context) {
        if (isSupported) {
            Log.w(
                "AndroidRsaCipherHelper",
                "Already initialised - Do not attempt to initialise this twice"
            )
            return
        }
        ...
        result = if (keyStore.containsAlias(alias)) {
            ...
            Log.v("AndroidRsaCipherHelper", "No keypair for $alias, creating a new one")
    ...
    private fun initAndroidM(alias: String): Boolean {
        try {
            ...
            Log.i(
                "AndroidRsaCipherHelper",
                "Random keypair with ${KeyProperties.KEY_ALGORITHM_RSA} ${KeyProperties.BLOCK_MODE_CBC} ${KeyProperties.ENCRYPTION_PADDING_RSA_PKCS1} is created."
            )
            ...
            Log.w(
                "AndroidRsaCipherHelper",
                "It seems that this device does not support RSA algorithm!!"
            )
    ...
    private fun initAndroidL(alias: String): Boolean {
        try {
            ...
            Log.i("AndroidRsaCipherHelper", "Random RSA algorithm keypair is created.")

            return true
        } catch (e: GeneralSecurityException) {
            Log.w(
                "AndroidRsaCipherHelper",
                "It seems that this device does not support encryption!!"
            )
